...
using BarracudaTensor = Unity.Barracuda.Tensor;
...
namespace MBaske.Sensors.Audio
{
    ...
    public class AudioSensor : ISensor, IDisposable
    {
        ...
        public Texture2D m_PerceptionTexture;
        ...
        public int Write(BarracudaTensor tensor)
        {
            int w = m_Shape.Width;
            int h = m_Shape.Height;
            ...
            for (int channel = 0; channel < m_Shape.Channels; channel++)
            {
                for (int x = 0; x < w; x++)
                {
                    for (int y = 0; y < h; y++)
                    {
                        tensor[0, y, x, channel] = Buffer.GetSample(channel, w * y + x);
                    }
                }
            }
